...
class SearchExport
{
    ...
    function marcfirst()
    {
        ...
        foreach ($this->results['record'] as $rawr) {
            $marc = $this->rutils->getMarcRecord($rawr);
            echo $marc->toRaw();
    ...
    function rdf($contenttype = "application/rdf+xml")
    {
        ...
        $interface->assign('marc', $this->rutils->getMarcRecord($r));
    ...
    {
        global $configArray;
        $m = $this->rutils->getMarcRecord($record);
        $lines = array();
        foreach ($this->tagSpecs['fieldmap'] as $exportTag => $tagspec) {
            foreach ($tagspec as $tuple) {
                ...
                if ($tag == 'TITLE') {
                    $titles = $this->rutils->getLongTitles($m);
                    $title = $titles[0];
    ...
    // Email records in the temporary selected set
    function emailTempRecords()
    {
    ...
    // Email a link to the serach plus the first few records of results
    function emailSearch()
    {
        ...
        $message .= "\n\nSearch:\n  "
            . "Terms: " . implode('; ', $this->ss->searchtermsForDisplay())
            . "\n  Facets: " . implode('; ', $this->ss->facetsForDisplay())
            . "\n  URL: "
            . $this->ss->asFullURL() . "\n\n The first several records are listed below\n";
    ...
    // Email an arbitrary set of records
    function emailRecords($to, $from, $message)
    {
        global $interface;
        global $configArray;
        $records = array();
        ...
        if (!Mail_RFC822::isValidInetAddress($to)) {
            echo json_encode(array('mailerror' => "To address '$to' is not valid<br>(must be of the form 'name@machine.com')"));
            return;
        }

        if (!Mail_RFC822::isValidInetAddress($from)) {
            echo json_encode(array('mailerror' => "From address '$from' is not valid<br>(must be of the form 'name@machine.com')"));
            return;
        }
...
//     $holdings = $this->rutils->getStatuses($this->results);
        ...
        foreach ($this->results['record'] as $r) {
            $marc = $this->rutils->getMarcRecord($r);
            $titles = $this->rutils->getLongTitles($marc);
            $id = $r['id'];
            $r['title'] = array_shift($titles);
            $r['othertitles'] = $titles;
            $r['url'] = $configArray['Site']['url'] . '/Record/' . $id;
            if (isset($r['author'])) {
                if (!is_array($r['author'])) {
                    $r['author'] = array($r['author']);
                }
                $r['authors'] = implode(', ', $r['author']);
            }

//       $h = $holdings[$id];
//       $r['holdings'] = $h;

            // Put in nothing for holdings
            $r['holdings'] = array();
        ...
        $body = $interface->fetch('Search/emailRecords.tpl');

        // $interface->display('Search/emailRecords.tpl');

        if (count($records) == 1) {
            $subject = $records[0]['title'];
        } else {
            $subject = "Mirlyn search results";
        }

        $headers['From'] = $from;
        $headers['To'] = $to;

        // Prepare for possible unicode subject
        ...
        $rv = $mail->send($to, $headers, $body);

        echo json_encode(array('rv' => $rv));
