...
<div>
   ...
   <div>
     <div class="record">
       {if $lastsearch}
        <a href="{$url}/Search/Home?{$lastsearch}"><img style="vertical-align: middle;" alt="" src="/static/umichwebsite/images/return.png">{translate text="Back to Search Results"}</a><br>
       {/if}
       
       <!-- fixme:suz email link doesn't work on phone as is... maybe there's another way to offer this functionality?
         <ul class="ToolLinks">
         <li><a href="#" id="emailRecord" class="mail">{translate text="Email this"}</a></li>
       </ul>
      -->
       ...
       <div class="recordnav">
         ...
         <h2>
         {if $previous}
         <span class="previous"><a href="{$url}{$previous}"><img style="vertical-align: middle;" alt="" src="/static/umichwebsite/images/arrow_left.png"> {translate text="Previous"}</a></span>
         {/if}
         {if $next}
         <span class="next"><a href="{$url}{$next}">{translate text="Next"} <img style="vertical-align: middle;" alt="" src="/static/umichwebsite/images/arrow_right.png"></a></span>
         {/if}
         </h2>
       </div> 
...
<table id="bibTable" summary="This table displays bibliographic information about this specific book or series">
...
<!-- some bib fields removed from mobile version for simplification
...
-->
  ...
  <tr valign="top">
    <th>{translate text='Print Version'}: </th>
    <td>
          {if is_array($record.oclc)}
<!-- title array -->
            {foreach from=$record.oclc item=title loop=1 name=loop}
              {if $smarty.foreach.loop.iteration lt 3}
              <a href="http://www.worldcat.org/oclc/{$title}">Find in a library</a><br>
              {/if}
            {/foreach}
          {else}
<!-- title non-array -->
            {if $record.oclc}
             <a href="http://www.worldcat.org/oclc/{$record.oclc}">Find in a library</a>
            {else} Find in a library service is not available from this catalog. <a href="http://www.worldcat.org/" target="_blank">Search Worldcat</a>
            {/if}
          {/if}
    </td>
  </tr>
  ...
  <ul>
    {foreach from=$marcField item=field name=loop}
       ...
       <!-- {assign var=nmspace value=$url|regex_replace:"/\.\d+/":""} -->
        ...
        <li><a href="http://hdl.handle.net/2027/{$url}" {if $field|getvalue:'r' eq 'pd'}class="fulltext">Full text{elseif $field|getvalue:'r' eq 'pdus'}class="fulltext">Full-text{elseif $field|getvalue:'r' eq 'world'}class="fulltext">Full text{else}class="searchonly">Search-only (no full-text){/if} <span class="IndItem">{if $field|getvalue:'z'}{$field|getvalue:'z'}{else}{/if}</span></a> 
     <!-- 'Original from XXX' removed from mobile version for simplification
       <em>
     {if $nmspace eq 'mdp'} (original from University of Michigan) {elseif $nmspace eq 'wu'} (original from University of Wisconsin)){elseif $nmspace eq 'inu'} (original from Indiana University{elseif $nmspace eq 'uc1'} (original from University of California){else}{/if}</li>
     </em>-->
    {/foreach}
  </ul>
</div>
