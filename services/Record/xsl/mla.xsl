...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template match="/">
                <xsl:if test="not(//datafield[@tag=245]/subfield[@code='h'])">
                    <!-- BOOK -->
    ...
    <xsl:template match="//datafield[@tag=700]">
        <xsl:if test="position() = last() and not(count(.) = 1)">
            ...
            <!-- xsl:value-of select="substring(//datafield[@tag=700]/subfield[@code='a'], 1, string-length(//datafield[@tag=700]/subfield[@code='a'])-1)"/ -->
            <xsl:value-of select="./subfield[@code='a']"/>
            <xsl:if test="not(//datafield[@tag=100])">
                <xsl:if test="substring(./subfield[@code='a'], string-length(./subfield[@code='a']), 1) != ','">
                    <xsl:text>, </xsl:text>
                </xsl:if>
