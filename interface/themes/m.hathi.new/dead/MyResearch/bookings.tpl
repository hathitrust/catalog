<div id="bd">
  <div class="yui-main content">
    ...
    {if $recordList}
      ...
      Use <a class="clickpostlog" ref="classicbookings" href="http://mirlyn-classic.lib.umich.edu">Mirlyn Classic</a> to create new bookings</p>
      {if $message}<p>{$message}</p>{/if}
      {if !is_array($recordList)}
        {translate text='You do not have any bookings placed'}.
      {else}
        {assign var="numItems" value=$recordList|@count}
        {if $numItems eq 1}
          {assign var="numdisp" value="1 booking"}
        {else}
          {assign var="numdisp" value="$numItems bookings"}
        {/if}
        <p>You have {$numdisp}</p>

        <ul class="filters">
        <form method="post">
        <input type="submit" name="submit" value="Delete bookings on checked items">
